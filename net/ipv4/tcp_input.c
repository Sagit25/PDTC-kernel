					 ...
					 const struct tcphdr *th)
{
	...
	printk(KERN_INFO "check puzzle for client ip: %u, dns ip: %u\n puzzle : %u, nonce : %u\n", ip_hdr(skb)->saddr, tp->rx_opt.dns_ip, tp->rx_opt.puzzle, tp->rx_opt.nonce);
	return do_check_puzzle(tp->rx_opt.puzzle_type, tp->rx_opt.puzzle, tp->rx_opt.dns_ip, tp->rx_opt.nonce, tp->rx_opt.threshold, ip_hdr(skb)->saddr);
}
/* Modified by shyang */
