...
u32 do_set_puzzle(__u32 nonce, __u32 seed, __u32 dns_ip, __u32 client_ip, __u32 puzzle_type) {
...
SYSCALL_DEFINE5(set_puzzle, __u32, nonce, __u32, puzzle, __u32, dns_ip, __u32, client_ip, __u32, puzzle_type) {
...
int do_check_puzzle(u32 type, u32 puzzle, u32 dns_ip, u32 nonce, u32 threshold, u32 client_ip) {
    struct puzzle_policy* policy;
    find_puzzle_policy(dns_ip, &policy);
    ...
    if (type == PZLTYPE_NONE)
        return 1;
    if (do_set_puzzle(nonce, puzzle, dns_ip, client_ip, puzzle_type) < policy->threshold)
       return 0;
    return 1;
}
...
u32 do_set_threshold(u32 ip, u32 threshold) {
	struct puzzle_policy* policy;
    do_set_puzzle_type(PZLTYPE_EXT);
...
u32 do_set_puzzle_type(u32 type) {
    ...
    if (type == PZLTYPE_NONE) return puzzle_type = type;
